...
tab1, tab2 = st.tabs(["Chat với model", "So sánh và đánh giá các model"])
        ...
        if not selected_batch_models:
            ...
            for sample in testset_data[:num_run]:
                ...
                for label, model_name in zip(selected_batch_labels_effective, selected_batch_models):
                    ...
                    try:
                        ...
                        judge_output = None
                        if judge_enabled:
                            status_text.text(f"Giám khảo đang chấm điểm: {label} - {question_id}...")
            ...
            if judge_enabled and judge_records:
                ...
                if available_hall_columns:
                    ...
                    st.subheader("Biểu đồ Tỷ lệ Ảo giác (Hallucination) theo Mô hình")
